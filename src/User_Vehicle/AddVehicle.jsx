...
import { doc, getDoc, setDoc, updateDoc } from 'firebase/firestore'; // Firestore functions for document operations
...
const AddVehicle = () => {
  ...
  // Function to handle availableFrom date change with validation

  // Function to handle availableTo date change with validation
  ...
  const handleSubmit = async (event) => {
    ...
    const selectedFromDate = new Date(availableFrom);
    const selectedToDate = new Date(availableTo);
    const currentDate = new Date();

    if (selectedFromDate < currentDate) {
      toast.error("Available From date cannot be earlier than today's date.");
      return;
    }
    if (availableFrom && selectedToDate < selectedFromDate) {
      toast.error("Available To date cannot be earlier than Available From date.");
      return;
    }

    try {
      // Only check for existing vehicle number if adding a new vehicle
      if (!vehicleData) {
        const vehicleDocRef = doc(db, 'VehicleDetails', vehicleNumber);
